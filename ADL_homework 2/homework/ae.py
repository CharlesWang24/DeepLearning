...
class PatchAutoEncoder(torch.nn.Module, PatchAutoEncoderBase):
    ...
    class PatchEncoder(torch.nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            x = hwc_to_chw(x)  # (B, latent_dim, H
            x = self.act(x + self.mix(x))  # (B, latent_dim, H//patch_size, W//patch_size)
            x = self.proj(x)  # (B, bottleneck, H//patch_size, W//patch_size)
            x = chw_to_hwc(x)  # (B, H//patch_size,
            return x
    ...
    class PatchDecoder(torch.nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            x = self.proj(x)  # (B, latent_dim, H//patch_size, W//patch_size)
            x = self.act(x + self.mix(x))  # (B, latent_dim, H//patch_size, W//patch_size)
            x = chw_to_hwc(x)  # (B, H//patch_size, W//patch_size, latent_dim)
            x = self.unpatchify(x)  # (B, H, W, latent_dim)
            return x
