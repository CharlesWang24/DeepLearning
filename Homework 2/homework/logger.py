...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        logger.add_scalar('train_accuracy', torch.cat(metrics["train_acc"]).mean(), global_step - 1)
        ...
        logger.add_scalar('val_accuracy', torch.cat(metrics["val_acc"]).mean(), global_step - 1)
